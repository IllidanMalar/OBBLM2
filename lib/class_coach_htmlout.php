...
public static function isAllowed($cid){
	global $coach;
    $c = new self($cid);
	return (is_object($coach) && ($c->coach_id == $coach->coach_id || $coach->mayManageObj(T_OBJ_COACH, $c->coach_id)));
}
private function _menu($ALLOW_EDIT)
{
	global $lng, $settings, $rules, $coach;
    if( !isset($_SESSION['logged_in']) || $coach->coach_id != $this->coach_id){	 /* Se non chi guarda non e' il propritario del profilo o se chi guarda non e' loggato */   
