...
public static function simpleLeagueSelector()
{
    ...
    $HTMLselector  = "<form class='simpleLeageSelector' name='SLS' method='POST' style='display:inline; margin:0px;'>";
	$HTMLselector .= "<div class='input-group'><span class='input-group-addon'>".$lng->getTrn('common/league')."</span>";
...
#    $leagues = Coach::allowedNodeAccess(Coach::NODE_STRUCT__TREE, is_object($coach) ? $coach->coach_id : false);
    ...
    <select class="form-control" <?php if ($hideNodes) {echo "style='display:none;'";}?> name="node" onChange="
    ...
    if ($setState) {
        ...
        ?>
        <select class="form-control" name="state_in" id="state_in">
            <?php
    ...
    if ($setRace) {
        ...
        ?>
        <select class="form-control" name="race_in" id="race_in">
            <?php
    ...
    if ($setSGrp) {
        ...
        ?>
        <select class="form-control" name="sgrp_in" id="sgrp_in">
            <?php
    ...
    if ($setFFilter) {
        ...
        ?>
        <select class="form-control" name="ffilter_field_in" id="ffilter_field_in">
            <?php
        ...
        <select class="form-control" name="ffilter_ineq_in" id="ffilter_ineq_in">
...
#    print_r($leagues);'
    ...
    // Done preprocessing...
    $NL = '';
    $NL .= "<select class='form-control' name='$nameid' id='$nameid' ".implode(' ', $extra_tags).">\n";
    if ($init_option) {
        $NL .= $init_option;
    }
    switch ($node) {
...
public static function frame_begin($stylesheet = false, $menu = true)
{
    ...
    ?>
    <!DOCTYPE html>
    <html <?php echo 'lang="'.substr($settings['lang'], 0, 2).'" '; ?>>
    <head>
        <meta charset="utf-8"> 
        <meta http-equiv="X-UA-Compatible" content="IE=edge">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <title><?php echo $settings['site_name']; ?></title>
        <link href="bootstrap/3_3_7/css/bootstrap.min.css" rel="stylesheet">
        <link href="bootstrap/3_3_7/css/bootstrap-theme.min.css" rel="stylesheet">        
    ...
    <body>
        <div class="everything container-fluid">
            <div class="banner jumbotron">
            	<div class="nomeLega"><h1><?php echo $settings['league_name']; ?></h1></div>
            </div>
            <nav class="navbar navbar-inverse"> <!-- Menu div -->
            	<div class="excontainer-fluid">
		          <div class="navbar-header">
		            ...
		            <button type="button" class="navbar-toggle collapsed" aria-expanded="false"><span class="testoBottone"><?php echo $settings['league_name']; ?></span></button>
		          </div>
            ...
            <div class="section container-fluid"> <!-- This container holds the section specific content -->
            	<div class="row">
    <?php
}
...
public static function frame_end()
{
    ?>
        ...
        <script src="bootstrap/3_3_7/js/bootstrap.min.js"></script>
    </body>
    </html>
    <?php
    return true;
...
private static function make_menu()
{
    ...
    <ul class="nav navbar-nav navbar-left">
      ...
      <?php if (isset($_SESSION['logged_in']) && is_object($coach)) {
            ...
            if (!empty($admin_menu)) { ?>
                <li class="dropdown">
                	<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><?php echo $lng->getTrn('menu/admin_menu/name');?> <span class="caret"></span></a>
                    ...
                    <?php foreach ($admin_menu as $lnk => $desc) {
                        if (!is_array($desc)) {
                        	...
                        	<li class="dropdown-header"><?php echo $desc['title'];?></li>   
                            	<?php foreach ($desc['sub'] as $sub) {
      ...
      <li class="dropdown">
      	<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><?php echo $lng->getTrn('menu/matches_menu/name');?> <span class="caret"></span></a>
            <ul class="dropdown-menu">
        ...
        <li class="dropdown">
        	<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><?php echo $lng->getTrn('menu/statistics_menu/name');?> <span class="caret"></span></a>
            <ul class="dropdown-menu">
        ...
        ?>
        <li class="dropdown">
        	<a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><?php echo $lng->getTrn('menu/plugins');?> <span class="caret"></span></a>
            <ul class="dropdown-menu">
        ...
        <li><a href="index.php?section=rosters"><?php echo 'Rosters';?></a></li>
        <li><a href="index.php?section=about">OBBLM</a></li>
        
        <?php if (!empty($settings['league_url'])) { ?>
            <li><a href="<?php echo $settings['league_url'];?>"><?php echo $settings['league_url_name'];?></a></li>
        <?php } ?>
        
        <?php //MARCO DA ?>
        <?php if ($settings['more_buttons'] > 0){
        	for ($i=0; $i < $settings['more_buttons']; $i++) { 
				echo '<li><a href="'.$settings['more_buttons_url'][$i].'">'.$settings['more_buttons_text'][$i].'</a></li>';
			}        	
        } ?>
        <?php //A MARCO ?>
    ...
    <ul class="nav navbar-nav navbar-right">
    	<?php if (isset($_SESSION['logged_in']) && is_object($coach)): ?>
    		<?php $ciccio = urlcompile(T_URL_PROFILE,T_OBJ_COACH,$coach->coach_id,false,false); ?> 
        	<li class="dropdown">
        		<a href="#" class="dropdown-toggle" style="font-weight:bold;" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><span class="glyphicon glyphicon-user" aria-hidden="true"></span> <?php echo ucfirst($coach->name); ?> <span class="caret"></span></a>
        		<ul class="dropdown-menu">
			        <li><a href="<?php echo $ciccio.'&amp;subsec=teams';?>"><?php echo $lng->getTrn('cc/coach_teams');?></a></li>
			        <li><a href="<?php echo $ciccio.'&amp;subsec=profile';?>"><?php echo $lng->getTrn('cc/profile');?></a></li>
			        <li><a href="<?php echo $ciccio.'&amp;subsec=stats';?>"><?php echo $lng->getTrn('common/stats');?></a></li>
			        <li><a href="<?php echo $ciccio.'&amp;subsec=recentmatches';?>"><?php echo $lng->getTrn('common/recentmatches');?></a></li>
			        
			        <?php if (Coach_HTMLOUT::isAllowed($coach->coach_id)) { ?>
			        	<li><a href="<?php echo $ciccio.'&amp;subsec=newteam';?>"><?php echo $lng->getTrn('cc/new_team');?></a></li>
			        <?php }
			        if (Module::isRegistered('SGraph')) {
            			echo "<li><a href='handler.php?type=graph&amp;gtype=".SG_T_COACH."&amp;id=".$coach->coach_id."'>Vis. stats</a></li>\n";
        			} ?>
        		</ul>
        	</li>
        <?php endif; ?>
    	...
    	<?php if (isset($_SESSION['logged_in'])) { ?>
        	<li><a href="index.php?logout=1"><span class="glyphicon glyphicon-log-out" aria-hidden="true"></span> <?php echo $lng->getTrn('menu/logout');?></a></li>
        <?php }
        else{ ?>
        	<li><a href="index.php?section=login"><span class="glyphicon glyphicon-log-in" aria-hidden="true"></span> <?php echo $lng->getTrn('menu/login');?></a></li>            
        <?php } ?>
    </ul>   
